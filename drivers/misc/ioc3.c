...
static int ioc3_nic_wait(struct ioc3_driver_data *idd)
{
	unsigned mcr;
...
static int ioc3_nic_reset(struct ioc3_driver_data *idd)
{
	int presence;
	unsigned long flags;
...
static int ioc3_nic_read_bit(struct ioc3_driver_data *idd)
{
	int result;
	unsigned long flags;
...
static void ioc3_nic_write_bit(struct ioc3_driver_data *idd, int bit)
{
	if (bit)
...
static unsigned ioc3_nic_read_byte(struct ioc3_driver_data *idd)
{
	unsigned result = 0;
	int i;
...
static void ioc3_nic_write_byte(struct ioc3_driver_data *idd, int byte)
{
	int i, bit;
...
static unsigned long ioc3_nic_find(struct ioc3_driver_data *idd,
				int *last, unsigned long addr)
{
	int a, b, index, disc;
...
static void ioc3_nic_addr(struct ioc3_driver_data *idd, unsigned long addr)
{
	int index;
...
static void ioc3_read_redir_page(struct ioc3_driver_data *idd,
				unsigned long addr, int page,
...
static void ioc3_read_redir_map(struct ioc3_driver_data *idd,
				unsigned long addr,
				unsigned char *redir)
{
...
static void ioc3_read_nic(struct ioc3_driver_data *idd, unsigned long addr)
{
...
static void ioc3_read_mac(struct ioc3_driver_data *idd, unsigned long addr)
{
	int i, loops = 3;
...
static void ioc3_probe_nic(struct ioc3_driver_data *idd)
{
	int save = 0, loops = 3;
...
static irqreturn_t ioc3_intr_io(int irq, void *arg)
{
	...
	for (id = 1; id < IOC3_MAX_SUBMODULES; id++)
		if ((idd->active[id]) &&
			...
			ioc3_write_ireg(idd, ioc3_submodules[id]->
				irq_mask, IOC3_W_IEC);
	...
	if(pending) {
		printk(KERN_WARNING
			...
			pending);
		ioc3_write_ireg(idd, pending, IOC3_W_IEC);
		handled = 1;
	}
...
{
	ioc3_write_ireg(idd, irqs & is->irq_mask, IOC3_W_IES);
}
...
void ioc3_disable(struct ioc3_submodule *is,
				struct ioc3_driver_data *idd, unsigned int irqs)
{
	ioc3_write_ireg(idd, irqs & is->irq_mask, IOC3_W_IEC);
}
...
void ioc3_gpio(struct ioc3_driver_data *idd,
...
int ioc3_register_submodule(struct ioc3_submodule *is)
{
	...
	if (is->ethernet)
		...
		alloc_id = find_slot((void **)ioc3_submodules,
					IOC3_MAX_SUBMODULES);
	...
	if(alloc_id == -1) {
		printk(KERN_WARNING "ioc3: Increase IOC3_MAX_SUBMODULES!\n");
		return -ENOMEM;
	}
	...
	list_for_each_entry(idd, &ioc3_devices, list)
		if(idd->active[is->id]) {
			if(is->remove)
				if(is->remove(is, idd))
					printk(KERN_WARNING
					"ioc3: submodule %s remove failed "
					"for pci_dev %s.\n",
					module_name(is->owner),
					pci_name(idd->pdev));
			idd->active[is->id] = 0;
			if(is->irq_mask)
				ioc3_write_ireg(idd, is->irq_mask, IOC3_W_IEC);
		}
	up_read(&ioc3_devices_rwsem);
}
...
static char *ioc3_class_names[] __devinitdata = {
				"unknown",
				"IP27 BaseIO",
				"IP30 system",
				"MENET 1/2/3",
				...
				const struct pci_device_id *pci_id)
{
	...
	if (pci_enable_device(pdev)) {
		printk(KERN_WARNING
			"ioc3: Failed to enable pci_dev %s.\n",
			pci_name(pdev));
		goto out;
	}
	...
	if (pci_request_regions(pdev, "ioc3")) {
		printk(KERN_WARNING
			"ioc3: Unable to request region for pci_dev %s.\n",
			pci_name(pdev));
	...
	if (!ret) {
		...
		if (ret < 0)
			printk(KERN_WARNING "ioc3: Unable to obtain 64 bit DMA "
	...
	if (!idd) {
		printk(KERN_WARNING
			"ioc3: Failed to allocate data for pci_dev %s.\n",
			pci_name(pdev));
		ret = -ENOMEM;
	...
	/* Clear IRQs */
	ioc3_write_ireg(idd, ~0, IOC3_W_IEC);
	...
	if (idd->class == IOC3_CLASS_BASE_IP30 ||
		...
		if (!request_irq(pdev->irq, ioc3_intr_eth, IRQF_SHARED,
			...
			printk(KERN_WARNING
				"ioc3: request_irq fails for IRQ 0x%x\n ",
				pdev->irq);
		}
...
static void __devexit ioc3_remove(struct pci_dev *pdev)
{
	...
	for (id = 0; id < IOC3_MAX_SUBMODULES; id++)
		if (idd->active[id]) {
			struct ioc3_submodule *sub = ioc3_submodules[id];
			if (sub && sub->remove)
				if (sub->remove(sub, idd))
					printk(KERN_WARNING "ioc3: submodule "
					"0x%s remove failed for pci_dev %s.\n",
					module_name(sub->owner), pci_name(pdev));
			idd->active[id] = 0;
		}
